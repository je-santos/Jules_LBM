...
def run_lbm_simulation_and_get_velocity(Lx, Ly, geometry, niu, rho_in, rho_out, total_iterations):
    """
    Initializes and runs the LBM simulation, then returns the velocity profile.
    Uses pressure (density) boundary conditions for x-direction.
    """
    solver = LB2D_Solver_Single_Phase(nx=Lx, ny=Ly)
    solver.init_geo(geometry)
    solver.set_viscosity(niu)
    # solver.set_force([fx, 0.0]) # Apply force in x-direction - REMOVED for pressure BC

    # Boundary conditions:
    # X-direction: Pressure (density) boundary conditions
    solver.set_bc_rho_x0(rho_in)  # Inlet at x=0
    solver.set_bc_rho_x1(rho_out) # Outlet at x=Lx-1
...
def plot_results(y_coords_lbm, lbm_velocities, analytical_velocities, Ly, H_channel):
    ...
    plt.title(f"Pressure-Driven Poiseuille Flow (Ly={Ly}, H_channel={H_channel})")
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    # fx_lbm = 1.0e-5   # Body force in x-direction (acts as G_effective if rho_avg=1) - REMOVED for pressure BC
    ...
    # G_eff = fx_lbm # REMOVED - G_eff will be based on pressure difference

    rho_in_val = 1.01  # Inlet density (pressure)
    rho_out_val = 0.99 # Outlet density (pressure)

    # For analytical solution based on pressure gradient
    cs_sq = 1.0 / 3.0  # Speed of sound squared in LBM units (D2Q9 model)
    delta_P_val = (rho_in_val - rho_out_val) * cs_sq  # Pressure difference
    G_eff_pressure = delta_P_val / Lx_grid  # Effective force gradient G = dP/dx = delta_P / Length_x
    ...
    lbm_velocity_profile, lbm_solver_instance = run_lbm_simulation_and_get_velocity(
        Lx_grid, Ly_grid, pipe_geom, niu_lbm, rho_in_val, rho_out_val, total_sim_iterations
    )
    # lbm_velocity_profile is for all y-nodes, including walls.

    # 3. Calculate Analytical Solution (Placeholder - G_eff needs to be redefined for pressure-driven flow)
    ...
    # ---- Analytical solution calculation reinstated ----
    analytical_velocity_profile = calculate_analytical_poiseuille(
        G_eff_pressure, niu_lbm, H_actual_channel, y_physical_coords_for_analytical
    )
    # ---- End of analytical solution calculation ----
    ...
    # 5. Calculate and Print Discrepancy (will be inaccurate due to placeholder analytical solution)
    ...
    if np.isnan(mean_rel_err):
        ...
        print(f"Maximum Relative Error: {max_rel_err:.6%}")
